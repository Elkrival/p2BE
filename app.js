...
app.get('/netflix/new', function (req, res) {
    console.log(req.params)
    db.collection(NETFLIX_N_CHILL_COLLECTION).find({}).toArray(function (error, result) {
        if (error) {
            console.log('No movies here!', error);
            res.json('No movies here');
        } else {
            // console.log('Displaying Movies' + res.json(result));******DO NOT DO THIS, THIS IS A METHOD********
            res.json(result);
        }
    })
})
